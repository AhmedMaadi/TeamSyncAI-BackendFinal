...
export async function getTasksByProjectID(req, res) {
    try {
        ...
        console.log('Fetching tasks for projectID:', projectID); // Debug statement
        const tasks = await Task.find({ projectID }).exec();
        console.log('Retrieved tasks:', tasks); // Debug statement
        res.status(200).json({ tasks });
    } catch (error) {
        console.error('Error retrieving tasks by projectID:', error);
        res.status(500).json({ error: 'Failed to retrieve tasks by projectID' });
    }
}
...
export async function createDefaultTask(projectID, module_id) {
    try {
        ...
        const newTask = new Task({ 
            module_id: module_id, 
            task_description: defaultTaskDescription, 
            projectID, 
            team: [], 
            Duration: null, 
            startDate: null, 
