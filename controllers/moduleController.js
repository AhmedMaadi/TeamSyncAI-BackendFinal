...
export async function receiveModules(req, res) {
    try {
        ...
        for (const moduleData of modules) {
            const { module_name, tasks, teamM } = moduleData; 
            
            const newModule = new Module({ module_name, projectID, teamM, moduleStartDate: null, moduleEndDate: null, totalDuration: null });
            await newModule.save();

            for (const task of tasks) {
                // Create a new task with default null values for new attributes
                const newTask = new Task({ 
                    ...
                    Duration: null, 
                    startDate: null, 
                    endDate: moment().toDate(), // Set default end date to today's date
                });
                await newTask.save();
            }
        }
...
export async function createDefaultModule(projectID) {
    try {
        ...
        // Create a new module with default null values for new attributes
